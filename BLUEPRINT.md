...
**typstR** is an R package for creating publication-ready scientific documents in **Quarto + Typst** with an R-native workflow.

It does **not** try to replace Quarto’s built-in Typst support. Instead, it builds on top of it and provides:
...
> “A polished R workflow for scientific and institutional PDF publishing with Quarto and Typst.”

### Why this package is useful
Quarto already supports Typst output and custom Typst formats, so the gap is not basic compatibility. The gap is a cohesive, reusable, R-friendly package that turns Quarto + Typst into a practical publication workflow. Existing related tools such as `typr` and `typstable` show that the ecosystem is real, but they do not by themselves provide a complete manuscript framework. 

### Primary target users
- researchers writing working papers
- economists and policy analysts
- institutes producing reports and briefs
- R users who want LaTeX-quality PDFs with a cleaner authoring experience
...
- validation
- render wrappers
- utility functions for notes, appendices, affiliations, etc.

### B. Quarto extension layer
Lives inside `inst/` and contains:
- `_extension.yml`
- format definitions
- partials
- template assets
- bundled Typst template files

### C. Typst template layer
Contains reusable Typst building blocks for:
- title page
...
### Example workflow
1. Create a project with `create_working_paper()`
2. Edit the generated `.qmd`
3. Fill metadata in YAML or helper functions
4. Render with `quarto_render()` or `typstR::render_pub()`
...
The package should support plain Quarto YAML with a small set of `typstR` fields.

### Example
```yaml
---
title: "Trade Fragmentation and Industrial Policy"
subtitle: "Evidence from Central and Eastern Europe"
author:
  - name: "Jane Doe"
    affiliation: "1"
...
typstR:
  abstract: |
    This paper studies...
  keywords: ["trade", "industrial policy", "CEE"]
  jel: ["F10", "F13", "L52"]
  acknowledgements: |
    We thank...
